...
class Spreadsheet_Excel_Reader {
	...
	function val($row,$col,$sheet=0) {
		//return "sadasdasdaã";iconv(mb_detect_encoding($text, mb_detect_order(), true), "UTF-8", $text);
		$test = "";
		$col = $this->getCol($col);
		if (array_key_exists($row,$this->sheets[$sheet]['cells']) && array_key_exists($col,$this->sheets[$sheet]['cells'][$row])) {
			$test = $this->sheets[$sheet]['cells'][$row][$col];
		}
		
		return iconv(mb_detect_encoding($test, mb_detect_order(), true), "UTF-8", $test);;
